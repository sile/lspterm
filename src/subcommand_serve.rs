...
pub fn try_run(mut args: noargs::RawArgs) -> noargs::Result<Option<noargs::RawArgs>> {
    if !noargs::cmd("serve").take(&mut args).is_present() {
        return Ok(Some(args));
    }
    ...
    let lsp_server_command: PathBuf = noargs::arg("LSP_SERVER_COMMAND")
        ...
        .take(&mut args)
        .then(|a| a.value().parse())?;
    let mut lsp_server_args: Vec<PathBuf> = Vec::new();
    while let Some(a) = noargs::arg("[LSP_SERVER_ARG]...").take(&mut args).present() {
        lsp_server_args.push(a.value().parse()?);
    }
